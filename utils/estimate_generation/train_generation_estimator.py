...
params = {							# parameters for training estimator
	'n_estimators': 1200,           # AKA number of trees; optimum = 1500 (?)
...
feature_name_list = ['Fuel type','Capacity (MW)','Commissioning year','Average CF (fuel)','Share nat. cap.','Share nat. cap. by fuel']
...
# display score
ax = plt.gca()
plt.text(0.65,0.36,"{0}".format(','.join(countries)),transform=ax.transAxes,fontsize=10,color='r')
plt.text(0.65,0.32,"Total observations: {0}".format(len(X_data_np)),transform=ax.transAxes,fontsize=10,color='r')	
plt.text(0.65,0.28,"N_estimators: {0}".format(params['n_estimators']),transform=ax.transAxes,fontsize=10,color='r')
plt.text(0.65,0.24,"Max depth: {0}".format(params['max_depth']),transform=ax.transAxes,fontsize=10,color='r')
plt.text(0.65,0.20,"Num. folds: {0}".format(num_folds),transform=ax.transAxes,fontsize=10,color='r')
plt.text(0.65,0.16,"Learning rate: {0}".format(params['learning_rate']),transform=ax.transAxes,fontsize=10,color='r')
plt.text(0.65,0.12,"Sub-sample: {0}".format(params['subsample']),transform=ax.transAxes,fontsize=10,color='r')
plt.text(0.65,0.08,"Loss: {0}".format(params['loss']),transform=ax.transAxes,fontsize=10,color='r')
plt.text(0.65,0.04,"R2: {:4.3f}".format(r2_score),transform=ax.transAxes,fontsize=10,color='r')
